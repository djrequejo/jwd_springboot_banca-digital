...
public interface MyBatisClienteMapper {
	...
	@Results({
			@Result(property = "usuario", column = "id", javaType = Usuario.class, one = @One(select = "com.mitocode.javaweb.banca_digital.usuario.infraestructure.persistence.MyBatisUsuarioMapper.findByIdCliente")),
			@Result(property = "tarjetas", column = "id", javaType = List.class, many = @Many(select = "com.mitocode.javaweb.banca_digital.tarjeta.infraestructure.persistence.MyBatisTarjetaMapper.findByIdCliente")) })
